...
/// A receipt builder optimized for reliable Arabic printing on thermal printers.
///
/// This builder exclusively uses raster (image-based) rendering for all text,
/// which guarantees correct Arabic shaping and right-to-left layout on printers
/// that may not have native Arabic font support.
class ArabicReceiptBuilder {
  ...
  /// Factory to initialize the capability profile and load the Arabic font.
  static Future<ArabicReceiptBuilder> create({
    ...
    _d(debug,
        'ARB.create() → loading profile "${profileName ?? 'default'}"...');
    final profile =
        await CapabilityProfile.load(name: profileName ?? 'default');
    final widthPx = widthPxOverride ?? ((paper == PaperSize.mm58) ? 384 : 576);
    _d(debug,
        'ARB.create() ✓ profile in ${sw.elapsedMilliseconds}ms; paper=$paper, widthPx=$widthPx');

    if (arabicFontFamily != null && arabicFontAssetPath != null) {
      try {
        final swFont = Stopwatch()..start();
        _d(debug,
            'ARB.create() → loading Arabic font "$arabicFontFamily" from "$arabicFontAssetPath"...');
        await ArabicFontLoader.ensureLoaded(
          family: arabicFontFamily,
          assetPath: arabicFontAssetPath,
        );
        _d(debug,
            'ARB.create() ✓ font loaded in ${swFont.elapsedMilliseconds}ms');
      } catch (e, st) {
        _e(debug, 'ARB.create() ✗ font load failed: $e\n$st');
      }
    } else {
      _w(debug,
          'ARB.create() ⚠ No Arabic font configured; set arabicFontFamily + arabicFontAssetPath.');
    }

    return ArabicReceiptBuilder._(
      paper,
      profile,
      widthPx,
      arabicFontFamily,
  ...
  Future<Uint8List> buildCustomer(
    ...
    // ---- Header ----
    bytes.addAll(await _arabicTextLine(
      g,
      'فاتورة',
    ...
    if (orderNo.isNotEmpty) {
      bytes.addAll(await _arabicKeyValueLine(
        g,
        label: 'رقم الطلب',
  ...
  Future<List<int>> _arabicTextLine(
    ...
    if (arabicFontFamily == null) {
      _w(debug,
          '_arabicTextLine() ⚠ arabicFontFamily == null (Arabic may break)');
    }
    ...
    try {
      ...
      _e(debug, '_arabicTextLine() ✗ build/layout failed: $e\n$st');
      rethrow;
    }
    ...
    try {
      ...
      _e(debug, '_arabicTextLine() ✗ rasterize failed: $e\n$st');
      rethrow;
    }
    // Decode → ESC/POS
  ...
  Future<List<int>> _arabicKeyValueLine(
    ...
    if (arabicFontFamily == null) {
      _w(debug, '_arabicKeyValueLine() ⚠ arabicFontFamily == null');
    }
    ...
    final pLabel = builderL.build()
      ..layout(ui.ParagraphConstraints(width: widthPx.toDouble()));
    // VALUE (LTR, right aligned across the full width)
    ...
    try {
      ...
      _e(debug, '_arabicKeyValueLine() ✗ rasterize failed: $e\n$st');
      rethrow;
    }
    final decoded = img.decodePng(pngBytes) ?? img.decodeImage(pngBytes);
    if (decoded == null) {
